...
- **The schema version answers to the *older than your data* guard, not only to the index list.** A release that adds a field the export mapper must learn owes a version even when `stores()` is unchanged — because the guard is the only thing standing between a rolled-back build and a Backup written with that field silently stripped. Such a version declares the same `stores()` and no `upgrade()`, which is legitimate rather than a smell. The deployment rule above keys on **the version, not the migration**: a no-op `version(n)` carries a version and no migration, and *never roll it back* still holds. The `Kick`'s `version(2)` is the first instance ([#95](https://github.com/dbgeek/vape-off/issues/95)).
