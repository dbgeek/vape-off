...
So the app treats installation as a precondition rather than a preference, and it spends its very first screen on it. A first run in a browser tab opens full-screen on the Share-sheet instruction, before any Puff Session can be written and before the *Start fresh / Restore from a backup* choice is offered at all.
...
- **The escape hatch is deliberately small but real.** "Continue anyway" is present on the first-run wall, because a hard block on a personal tool is a tool that cannot be tried. It is the only way past, and taking it lands the user directly in the permanently-barred state.
